...
class LazySearchingTest {
    ...
    @Test fun f3() {
        val a = infiniteSequence()
        val b = infiniteSequence().map { it * it }
    ...
    @Test fun f4() {
        val perfectCubes = infiniteSequence().map { it * it * it }
        val powersOf2 = infiniteSequence().filter { it and (it - 1) == 0 }
        val atLeastAsLargeAs20 = infiniteSequence(start = 20)
    ...
    @Test fun f5() {
        ...
        val c = infiniteSequence(start = 10)
        val input = listOf(a, b, c)
